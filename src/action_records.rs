...
impl BasicAction {
    ...
    pub fn compute_talon_script(&self) -> String {
        let arguments_text = self
            .compute_arguments_converted_to_talon_script_string()
            .join(", ");
...
impl Command {
    ...
    pub fn to_string(&self) -> String {
        let actions_text: Vec<String> =
            self.actions.iter().map(|action| action.to_json()).collect();
        ...
        format!(
            "Command({}, {}, {}",
            self.name, seconds_since_last_text, actions_joined
        )
    }
...
fn parse_basic_action_json_argument_element(
    text: &str,
    is_string: bool,
    ...
    if let Ok(i32) = trimmed_text.parse::<i32>() {
        ...
        let message = format!(
            "Invalid JSON element: {}, is string: {}",
            trimmed_text, is_string
        );
...
fn add_current_item(
    ...
    if value_text.is_empty() {
        let message = format!(
            "called add_current_item with key: |{}|, value_text: |{}|, is_current_value_string: {}\n",
    ...
    if key == "name" {
        ...
        let argument =
            parse_basic_action_json_argument_element(value_text, *is_current_value_string)?;
...
pub fn load_basic_action_from_json(json: &str) -> Result<BasicAction, String> {
        ...
        if is_inside_string {
            ...
            if is_inside_list {
                return Err(String::from(
                    "JSON string has nested lists, which is not permitted",
                ));
            } else if key != "arguments" {
                return Err(format!(
                    "JSON string has a list without a key 'arguments', found: {}",
                    key
                ));
            }
            is_inside_list = true;
            ...
            if !key.is_empty() || !current_text.is_empty() || is_current_value_string {
                add_current_item(
                    ...
                    &mut key,
                    &mut current_text,
                    &mut is_current_value_string,
                    is_inside_list,
                    &mut capture_name,
                    &mut capture_instance,
                    &mut unclosed_opening_braces,
                )?;
            }

            if is_inside_list && unclosed_opening_braces == 1 {
                arguments.push(Argument::CaptureArgument(TalonCapture::new(
                    &capture_name,
                    capture_instance,
                )));
                capture_name.clear();
            ...
            if !is_inside_list {
                return Err(String::from(
                    "JSON string has a closing bracket without an opening bracket",
                ));
            }
            ...
            if !key.is_empty() && !is_inside_list {
                return Err(format!(
                    "JSON string has a colon with a predefined key: {}",
                    key
                ));
            }
            if unclosed_opening_braces == 0 {
                return Err(String::from(
                    "JSON string has a colon without an opening brace",
                ));
            }
    ...
    if is_inside_string {
        ...
        return Err(String::from(
            "JSON string ends with an incomplete key-value pair",
        ));
    }
...
fn compute_command_name_without_prefix(name: &str) -> Result<String, String> {
    if name.starts_with(COMMAND_NAME_PREFIX) {
        ...
        if name_without_prefix.is_empty() {
            return Err(format!(
                "Command name text is empty after removing prefix '{}'",
                COMMAND_NAME_PREFIX
            ));
        }
        ...
        let message = format!(
            "Command name text does not start with prefix '{}': {}",
...
fn compute_seconds_since_last_action(time_record: &str) -> Result<u32, String> {
    if time_record.starts_with(TIME_DIFFERENCE_PREFIX) {
        ...
        Err(format!(
            "Time record does not start with '{}': {}",
            TIME_DIFFERENCE_PREFIX, time_record
        ))
    }
}
...
impl<'a> RecordParser<'a> {
    ...
    fn process_time_difference(&mut self, line: &str) -> Result<(), String> {
        ...
        self.seconds_since_last_action_for_next_command =
            Some(compute_seconds_since_last_action(line)?);
...
mod tests {
    ...
    fn assert_actions_match(action: &BasicAction, name: &str, arguments: &Vec<Argument>) {
        ...
        for (i, expected_argument) in arguments.iter().enumerate() {
            ...
            assert_eq!(
                ...
                "Argument at index {} does not match",
                i
            );
        }
    }
    ...
    fn test_insert_capture_action() {
        let name = String::from("insert");
        let arguments = vec![Argument::CaptureArgument(TalonCapture::new(
            "capture_name",
            1,
        ))];
    ...
    fn test_mouse_move_action() {
        ...
        let arguments = vec![Argument::IntArgument(100), Argument::IntArgument(200)];
    ...
    fn test_mouse_click_action() {
        ...
        let arguments = vec![Argument::IntArgument(1)];
